...
test(
  ...
  () => {
    ...
    assert.ok(
      SOURCE.includes(
        'retired after its single production invocation'
      )
    );
  }
);
...
test(
  ...
  () => {
    ...
    const result =
      loaded.context
        .reosCountyCollapseGroup2StrandedOperationReconciliationStatusV2();
    ...
    assert.strictEqual(
      result.residualExecutionBlocked,
      false
    );

    assert.ok(
      Array.isArray(
        result.residualExecutionBlockers
      )
    );

    assert.strictEqual(
      result.residualExecutionBlockers.length,
      0
    );

    assert.strictEqual(
      result.targetBoundPreparedOperationHistoryPresent,
      true
    );
...
test(
  'consumed V1 RPC is permanently retired before any read',
  () => {
    const loaded =
      load();
...
test(
  'V2 RPC accepts no caller-defined authority',
  () => {
    const loaded =
      load();
...
test(
  'prepared-only residual executionBlocked drift fails closed',
  () => {
    const residual =
      residualFixture();
    ...
    assert.throws(
      () =>
        loaded.context
          .reosCountyCollapseGroup2StrandedOperationReconciliationStatusV2(),
      /incident-bound prepared-only state/
    );
  }
);

test(
  'prepared-only residual uncertain-operation drift fails closed',
  () => {
    const residual =
      residualFixture();
    ...
    assert.throws(
      () =>
        loaded.context
          .reosCountyCollapseGroup2StrandedOperationReconciliationStatusV2(),
      /must not be classified as uncertain/
    );
  }
);

test(
  'prepared-only residual blocker drift fails closed',
  () => {
    const residual =
      residualFixture();
    ...
    assert.throws(
      () =>
        loaded.context
          .reosCountyCollapseGroup2StrandedOperationReconciliationStatusV2(),
      /must not expose residual execution blockers/
    );
  }
);
...
console.log(
  'GROUP2_STRANDED_RECONCILIATION_V2_BEHAVIOR_CASES=12'
);
...
console.log(
  'GROUP2_STRANDED_OPERATION_RECONCILIATION_V2_VALIDATION_PASSED=true'
);
